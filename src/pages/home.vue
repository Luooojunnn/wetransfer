<template>
  <div class="page">
    <div class="main">
      <div class="notic row">
        <img src="../assets/bct.png" alt="" class="notice-img" />
        <p>Notice：Important message notification window</p>
      </div>
      <img src="../assets/home-title.png" alt="" class="home-title" />
      <div class="home-position">
        <div class="position-title row">
          <img src="../assets/bct.png" alt="" class="notice-img" />
          <p>Your position</p>
        </div>
      </div>
      <div class="bct row">
        <div class="row">
          <img src="../assets/bct.png" alt="" class="bct-img" />
          <p>BCT/usdt</p>
        </div>
        <div class="row">
          <p>5MDX+6DEMA</p>

          <img src="../assets/bct.png" alt="" class="gift-img" />
        </div>
      </div>
      <div class="bct-box">
        <table class="table">
          <tr>
            <th width="15%">#</th>
            <th width="20%">Asset</th>
            <th width="15%">Debt</th>
            <th width="15%">APY</th>
            <th width="15%">Risk</th>
            <th width="20%"></th>
          </tr>
          <tr>
            <th>12347</th>
            <th>436,560,460.71 USDT</th>
            <th>2.50k USDT</th>
            <th>350.12 %</th>
            <th>60.12 %</th>
            <td class="row">
              <div class="table-btn">Add</div>
              <div class="table-btn">Redeem</div>
            </td>
          </tr>
        </table>
      </div>
      <div class="farm-title">Farm</div>
      <div class="farm-content">
        <div class="farm-left">
          <div class="farm-left-top row">
            <p>Leverage</p>
            <p>Leverage-Plus</p>
          </div>
          <ul class="farm-list row">
            <li class="farm-active">ALL</li>
            <li>USDT</li>
            <li>BUSD</li>
            <li>Others</li>
          </ul>
        </div>
        <div class="farm-right">
          <div class="farm-ipt-box">
            <input type="text" class="farm-ipt" />
          </div>
          <div class="farm-right-bottom">
            <img src="../assets/bct.png" alt="" class="farm-img" />
            <p>Impermanent Loss Calculator</p>
          </div>
        </div>
      </div>

      <table class="farm-table">
        <tr>
          <th width="30%">Pool</th>
          <th width="30%">tvl</th>
          <th width="15%">DEMA</th>
          <th width="15%">APY</th>
          <th width="10%"></th>
        </tr>
        <tr>
          <th>BCT/usdt</th>
          <th>256,571,210.52</th>
          <th>9,000</th>
          <th>2033.12%</th>
          <td class="row">
            <div class="table-btn">3x Farm</div>
          </td>
        </tr>
      </table>
    </div>
    <div class="bottom row">
      <p>Docs</p>
      <p>faq</p>
      <p>Audit</p>
      <div class="row">
        <img src="../assets/bct.png" alt="" class="bottom-icon" />
        <img src="../assets/bct.png" alt="" class="bottom-icon" />
      </div>
    </div>
  </div>
</template>

<script>
export default {
  data() {
    return {};
  },
};
</script>

<style>
.row {
  display: flex;
  flex-direction: row;
  align-items: center;
}
.page {
  background: #000;
  color: #fff;
}
.main {
  width: 1040px;
  margin: 0 auto;
  padding-top: 44px;
  padding-bottom: 80px;
}
.notic {
  width: 84%;
  height: 50px;
  line-height: 50px;
  margin: 0 auto 46px;
  font-size: 24px;
  justify-content: center;
  background: url("../assets/bg.png") center center no-repeat;
  background-size: cover;
}
.notice-img {
  width: 23px;
  height: 27px;
  margin-right: 42px;
}
.home-title {
  width: 549px;
  margin: 0 auto;
  margin-bottom: 43px;
  display: block;
}
.position-title {
  margin-bottom: 12px;
}
.bct {
  font-size: 24px;
  margin-bottom: 9px;
  justify-content: space-between;
}
.bct-img {
  margin-left: 27px;
  width: 49px;
  height: 30px;
  margin-right: 12px;
}
.gift-img {
  width: 57px;
  height: 50px;
  margin-left: 12px;
}
.bct-box {
  padding: 18px 42px 42px 27px;
  border: 1px solid #e539ff;
  border-radius: 20px;
  margin-bottom: 70px;
}
.farm-table,
.table {
  width: 100%;
}
.table-btn {
  color: #9439ff;
  min-width: 80px;
  padding: 12px;
  border: 1px solid #9439ff;
  border-radius: 16px;
  text-align: center;
}
.table td .table-btn:first-child {
  margin-right: 17px;
}
.farm-title {
  font-size: 48px;
  font-weight: bold;
  margin-bottom: 12px;
}
.farm-content {
  display: flex;
  justify-content: space-between;
  align-items: flex-end;
  margin-bottom: 40px;
}
.farm-left {
  font-size: 30px;
  font-weight: bold;
}
.farm-left-top p:first-child {
  color: #9439ff;
  margin-right: 30px;
}
.farm-list {
  margin-top: 23px;
}
.farm-list li {
  font-size: 20px;
}
.farm-list .farm-active {
  font-size: 26px;
  color: #9439ff;
}
.farm-list li:not(:last-child) {
  margin-right: 30px;
}
.farm-ipt-box {
  margin-bottom: 22px;
}
.farm-ipt {
  width: 388px;
  height: 35px;
  border: 1px solid #9439ff;
  border-radius: 10px;
  background: rgba(148, 57, 255, 0.1);
}
.farm-right-bottom {
  margin-right: 30px;
  display: flex;
  align-items: flex-end;
  justify-content: flex-end;
}
.farm-right-bottom .farm-img {
  width: 20px;
  height: 30px;
  margin-right: 22px;
}
.farm-right-bottom p {
  font-size: 20px;
  text-decoration: underline;
}
.farm-table tr {
  border: 1px solid #9439ff;
}
.bottom {
  width: 100%;
  height: 80px;
  background: #080808;
  font-size: 20px;
  color: #6b6b6b;
  position: fixed;
  left: 0;
  bottom: 0;
  justify-content: center;
}
.bottom p {
  margin-right: 14px;
}
.bottom .row {
  margin-left: 16px;
}
.bottom-icon {
  width: 30px;
  height: 30px;
  border-radius: 50%;
  margin-right: 8px;
}
</style>